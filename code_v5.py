...
class ParticleFilter:
    ...
    def observation_model(self, x, z):
        return np.exp(-0.5 * np.linalg.norm(x - z, axis=1) ** 2 / self.observation_std ** 2)
...
def calculate_bounds(particles, weights, action, z_next, transition_model, observation_model, m, As_k, As_k_plus_1):
    ...
    for i, (x_i, w_i) in enumerate(zip(particles, weights)):
        ...
        if i not in As_k_plus_1:
            ...
            sum_term = sum(transition_model(x_i, x_j, action) * w_j for x_j, w_j in zip(particles, weights))
            lower_bound -= w_i * np.log(P_z_x_i * sum_term)
    for x_i, w_i in zip(particles, weights):
        sum_term = sum(observation_model(x_j, z_next) * transition_model(x_j, x_i, action) * w_j for x_j, w_j in zip(particles, weights) if j in As_k)
